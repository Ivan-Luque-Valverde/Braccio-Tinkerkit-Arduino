...
def launch_setup(context: LaunchContext) -> List[LaunchDescriptionEntity]:
    ...
    move_group = BraccioMoveGroupMixin.node_move_group(
            parameters=[
    ...
    pick_and_place_node = Node(
        package="braccio_moveit_config", 
        executable="pick_and_place.py",
        name="braccio_pick_and_place",
        output="screen",
    )

    pick_and_place_event_handler = RegisterEventHandler(
    OnProcessStart(
        target_action=move_group,
        on_start=[pick_and_place_node],
        )
    )   
    ld.add_action(pick_and_place_event_handler)
